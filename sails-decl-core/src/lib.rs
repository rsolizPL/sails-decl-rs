...
use swc_ecmascript::ast::{
    Expr, Ident, Lit, PropName, Str, TsInterfaceBody, TsKeywordType, TsKeywordTypeKind,
...
pub fn gen_decl_for_model(
    code: String,
    model_name: String,
) -> Result<TsInterfaceDecl, GenDeclForModelError> {
    ...
    }).ok_or(GenDeclForModelError::IsNotCommonJsModule)?;
    ...
    }).ok_or(GenDeclForModelError::InvalidModel)?;
    ...
    for attribute in &attributes_obj.props {
        ...
        if type_annotation.is_err() {
            return Err(GenDeclForModelError::SDTypeHintParseError);
        }
...
fn get_prop_as_str(prop: &PropName) -> Option<&str> {
    match prop {
...
fn parse_type_hint(type_hint: &str) -> Result<TsType, ()> {
